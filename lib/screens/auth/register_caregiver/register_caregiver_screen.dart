...
class _RegisterCaregiverScreenState extends State<RegisterCaregiverScreen> {
  ...
  String? _validatePhone(String? value) {
    ...
    if (phone.length != 11) return 'Telefone inválido (11 dígitos)';
    return null;
  }
  ...
  Future<void> _signUp() async {
    ...
    try {
      ...
      debugPrint("Erro geral (inesperado) no cadastro cuidador: $error");
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
                ...
                TextButton(
                  onPressed: () {
                    Navigator.of(context).pushReplacement(
                      MaterialPageRoute(
                        builder: (context) => const RegisterFamilyScreen(),
                      ),
                    );
                  },
